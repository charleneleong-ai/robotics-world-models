...
class MockWorldModel:
    ...
    def predict(self, obs: torch.Tensor, action: torch.Tensor) -> torch.Tensor:
        # Simple prediction: next state = current state + action effect
        return obs + action.mean(dim=-1, keepdim=True).expand_as(obs) * 0.1
...
class TestCausalAttribution:
    ...
    def test_contact_failure_detection(self):
        ...
        obs = torch.randn(1, 10)
        action = torch.randn(1, 4)
        # Make predicted_next different from actual_next to trigger failure detection
        predicted_next = obs + 0.1
        actual_next = obs + 0.5  # Large difference to trigger contact failure
...
class TestRecoveryStrategies:
    ...
    def test_mechanism_specific_recovery(self):
        ...
        # Check that it's a contact-related strategy
        assert "contact" in result.strategy_used.lower() or "force" in result.strategy_used.lower()
        ...
        assert "visual" in result.strategy_used.lower() or "resample" in result.strategy_used.lower()
...
class MockEnvironment:
    ...
    def __init__(self, obs_dim: int = 10, max_steps: int = 10):
    ...
    def reset(self):
        self.current_step = 0
        return torch.randn(1, self.obs_dim)

    def step(self, action):
        self.current_step += 1
        next_obs = torch.randn(1, self.obs_dim)

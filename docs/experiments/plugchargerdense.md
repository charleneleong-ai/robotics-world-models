...
- **Honest wall — the reactive actor never grasps.** On even trivial **PickCube**, DreamerV3 gets **0 success**: the world model learns cleanly and the actor sharpens (score ~8, entropy collapses), but instrumentation showed **~97% of episodes never grasp the cube** (`is_grasped` ≈ 0.02). It converges to a *reach-and-hover* optimum of the dense reward and never closes the gripper; placement/static never trigger because the sequence never gets past grasp. An EE-control fix didn't help — the bottleneck is upstream of control mode.

**The diagnostic contrast that matters:** TD-MPC2 solves the *identical* env + reward (PickCube trivial, StackCube 1.0, PegInsertion 0.84). The difference is *how the action is chosen*: TD-MPC2's short-horizon **MPC planning** discovers the grasp→lift→place *sequence*, whereas DreamerV3's amortized **imagination actor** won't stumble into grasp acquisition from reward alone. So the generative-WM ladder-comparison is blocked not by throughput or the world model — both work — but by **grasp exploration in the reactive policy**. The next lever (deferred, not pursued) targets that specifically: demo/BC warm-start of the actor, a grasp-stage curriculum, or planning-augmented action selection.

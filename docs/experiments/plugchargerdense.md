...
**Verdict — necessary but not sufficient.** Value-aware model learning is directionally correct: it repairs the model's success-blindness ~halfway (a clean positive confirmation of the PR #10 mechanism), but PickCube success needs *both* a sharper reward-cliff model (terminal-frame oversampling / stronger weighting) *and* a policy that can place. This resolves the generative-WM thread as a **coupled model + policy problem** — with the first remaining lever, sharpening the reward cliff, tested next.

## Follow-up 3 — the reward cliff cannot be sharpened: an RSSM representation wall

Follow-up 2 left one model-side lever open — push the ~0.47 plateau into the true ~1.0 cliff by getting *more* success frames into the reward gradient. We took it to the limit: a `SUCCESS_HOLD=30` augmentation that re-emits each demo's terminal success obs (reward≈1.0) for 30 extra non-terminal steps — **16× the success-frame density** (~15% of the demo batch) — *combined with* tripled reward weighting (`×(1 + 30·reward + 50·|Δreward|)`, up from 10/30). Retrained the PickCube world model to 350k steps (`wm-va2`), matched to `wm-va`'s depth, reward loss converged.

**The head barely moved — and the asymmetry is the finding.** Teacher-forcing the success demo through `wm-va2`, the reward head predicts **0.542 at the true-1.0 success state** (vs `wm-va`'s 0.512) while nailing the *dense* grasped reward almost exactly (**0.455 vs true 0.465**). So the two hardest available levers, stacked, bought **~+0.03** at success — a widened-but-still-soft gap of 0.087 (vs 0.057), nowhere near a cliff. The model fits the *smooth* reward field perfectly and **saturates on the sparse terminal one no matter how hard the loss pushes.** The CEM planner confirms it behaviorally — 24 episodes, **success 0.042, placed 0.042** (identical to `wm-va`; grasp even drops, 0.62 vs 0.92) — the single success is a flag-logger miss present in both runs, not a real gain.

**The terminal result — the bottleneck is representational, not the loss.** Loss re-weighting and oversampling both act on the *training signal*; that they move the head ~+0.03 while the dense reward is fit to <0.01 error means the limit is **what the RSSM latent can represent**, not what the reward head is asked to fit. Under DreamerV3's observation-reconstruction pressure, the success state — <1% of experience even at 16× oversampling — is not made *separable* in `z` from the grasped/hovering cloud it is embedded among, so the reward head cannot assign it a distinct value however it is weighted. Value-aware learning flips the gradient's *sign* (0.09 → steering-away, to 0.54 → holding-through) but cannot carve a *basin*. This is the honest floor of the generative-WM arc: on this RSSM, a reactive actor and a CEM planner are both capped upstream by a world model that structurally cannot see a payoff its own policy never visits — a **coverage↔accuracy coupling that is representational, not merely a matter of loss weighting** ([spec](../specs/2026-07-19-value-aware-model-learning.md)).
...
## Verdict
The value is the *shape* of the result: on extreme-precision keyed insertion, **paradigm success is ordered by prior knowledge and by how the action is chosen**. RL asked to *discover* the maneuver from reward fails categorically (every fixable cause ruled out: sparse→dense, farming→farm-proof, dilution→persistent buffer, joint→EE, from-scratch→RLPD). Imitation — handed demonstrations — is the only learned paradigm that inserts, and it *scales* but plateaus (~0.17) below the privileged 0.56. The generative-WM thread adds three honest results: a ~42× throughput engineering win; a precise negative — a reactive imagination actor stalls one stage short even with success demos in every batch; and a *diagnosed* one — value-aware learning flips the reward gradient's sign toward success but cannot sharpen it into a cliff, because the RSSM under reconstruction pressure won't allocate a distinct latent basin to a state its own policy never visits (a coverage↔accuracy wall that is representational, not a loss-weighting artifact). The clean, general takeaway: **for extreme-precision contact tasks, imitate a demonstrator — and if you must learn online, plan the action sequence; a reactive actor optimizing shaped reward will settle for the reachable local optimum.**

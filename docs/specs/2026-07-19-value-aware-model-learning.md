...
**Status:** ✅ EXECUTED (2026-07-19) — **PARTIAL**, then closed **NO-GO** by the sharpening follow-up (2026-07-20): the value-aware lever flips the reward gradient toward success but the cliff **cannot be sharpened** — a representational wall, not a loss-weighting one. **Date:** 2026-07-19 → 2026-07-20.

> **Result:** Up-weighting the reward loss (10× + per-frame `×(1+10·reward+30·|Δreward|)`) plus 50% demo-success oversampling in the WM batches moved the reward head's prediction at the demo success state from **0.09 → 0.47 (~5×)** and flipped the gradient from steering-away to holding through the success region — confirming the wall is loss-addressable. But it under-predicts the full 1.0 cliff (a ~0.47 plateau; success frames are still ~0.7% of batches), and task success stays 0 because the actor never learned to *place* (0.035). So value-aware learning is **necessary but not sufficient**.
>
> **Sharpening follow-up (2026-07-20) — NO-GO:** the one remaining model-side lever, pushing the plateau into a cliff, was taken to the limit — `SUCCESS_HOLD=30` (16× success-frame density, ~15% of the demo batch) *plus* tripled reward weighting (`×(1+30·reward+50·|Δreward|)`), retrained to 350k (`wm-va2`). The reward head at success moved only **0.512 → 0.542** (dense grasped reward fit near-exactly, 0.455 vs 0.465), and the CEM planner stayed at **success 0.042 / placed 0.042**. The head saturates on the sparse terminal reward however hard the loss pushes → the bottleneck is **the RSSM representation** (the success state, <1% of experience, is not made separable in `z`), not the loss. Full result folded into [`plugchargerdense.md`](../experiments/plugchargerdense.md#follow-up-3--the-reward-cliff-cannot-be-sharpened-an-rssm-representation-wall).

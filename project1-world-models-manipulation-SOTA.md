...
**Honest compute reality:** training any frontier video world model from scratch (Genie 11B, UniSim, Cosmos pretrain, GAIA) is **off the table solo**. Realistic paths: **inference + LoRA on Cosmos**, **train-from-scratch on small academic WMs (DIAMOND / 1xgpt)**, or **fine-tune a small head/adapter (Vidar, iVideoGPT)**.
